...
// Geräte vom Typ 3EM
async function fetch3EMData(ipAddress) {
    const response = await axios.get(`http://${ipAddress}/rpc/EMData.GetStatus?id=0`).catch(error => {
        throw new Error(`Fehler beim Abrufen der Daten: ${error.message}`);
    });
    // Extrahiere die Verbrauchswerte und den Timestamp
    const meterReading = response.data.total_act;
...
async function fetchDimmer2Data(ipAddress) {
    ...
    const timestamp = response.data.meters[0].timestamp;
    return { meterReading, timestamp };
}
